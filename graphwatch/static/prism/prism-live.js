...
*/
(async function () {
  const CURRENT_URL = document.currentScript
    ...
    constructor(source) {
        ...
        keydown: (evt) => {
          if (evt.key == "Tab" && !evt.altKey) {
            ...
            if (
              ...
              this.textarea.value[this.selectionEnd] == evt.key
            ) {
      ...
      requestAnimationFrame(() => {
        ...
        this.textarea.spellcheck =
          this.source.spellcheck || sourceCS.getPropertyValue("--spellcheck");
      });
  ...
  $.ready().then(() => {
    ...
    ).forEach((source) => _.create(source));
  });
})();
